...
def _clientHandler_(request, object, complement):
    ...
    if request == 0:
        try:
            for i in range(0, 6):
                complement[i] = int(complement[i].get())
        except:
...
def clickEntered(click):
    clickPosition = {"x": click.x, "y": click.y}
    for i in range(0, len(_keepTrack)):
        print(click.x, click.y)
        if keepBoxes[_keepTrack[i]]["data"]["absolutePosition"][0] <= int(click.x) and keepBoxes[_keepTrack[i]]["data"]["absolutePosition"][0] + keepBoxes["x=0, y=0"]["data"]["absolutePosition"][0]  >= int(click.x) and keepBoxes[_keepTrack[i]]["data"]["absolutePosition"][1] <= int(click.y) and keepBoxes[_keepTrack[i]]["data"]["absolutePosition"][1] + keepBoxes["x=0, y=0"]["data"]["absolutePosition"][1] <= int(click.x):
            print("got it")
            keepBoxes[_keepTrack[i]]["data"]["isFull"] = True
            mainCanvas.itemconfig(keepBoxes[_keepTrack[i]]["data"]["boxObject"], fill="white")
            print(keepBoxes[_keepTrack[i]])
            break
    print(_keepTrack)
...
def initForm():
    ...
    mainCanvas.bind("<Button-1>", clickEntered)
    ...
    for i in range(0, (windowData["windowWidth"] // 20)):
        for v in range(0, windowData["windowHeight"] // 20):
            ...
            if digit ==1:
                ...
                keepBoxes['x=' + str(i) + ', y=' + str(v)] = {"position" : {"x" : i, "y" : v}, "data": {"isFull": digit, "boxObject": rectangle, "absolutePosition": [i*20, v*20, (i*20)+20, (v*20) + 20]}}
                _keepTrack.append('x=' + str(i) + ', y=' + str(v))
            else:
                digit = 0
